...
class CreatePembelianView extends StatefulWidget {
  const CreatePembelianView({Key? key}) : super(key: key);

  @override
  State<CreatePembelianView> createState() => _CreatePembelianViewState();
}

class _CreatePembelianViewState extends State<CreatePembelianView> {
  ...
  Future<void> _selectSupplierFromDialog(BuildContext context) async {
    ...
    showDialog<Supplier>(
      ...
      builder: (BuildContext context) {
        final supplierList = Provider.of<SupplierController>(context).dataSupplier;
        return SupplierSelectDialog(
          supplierList: supplierList,
          onSupplierSelected: (selected) {
            setState(() {
              selectSup = selected;
              namaCustomer.text = "${selected.kodeSupplier} - ${selected.namaSupplier}"; 
            });
          },
        );
      },
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    validator: (value) => value!.isEmpty ? 'Supplier is required' : null,
                  ),
                  SizedBox(height: 10),
                  Divider(thickness: 2,),
                  SizedBox(height: 10),
                      ...
                      ElevatedButton.icon(
                        onPressed: () {
                          ...
                          if (listItems.length>0 && noPo.text!="" && namaCustomer.text!="") {
                            Pembelian pembelian = Pembelian(no_po: noPo.text, supplier: selectSup!.kodeSupplier, supplierName: selectSup!.namaSupplier, tanggal: selectedDate, item: listItems, jumlah: totalInv, status: false);
                            pembelianCtrl.addNewPembelian(pembelian).then((value) => {
                            if (value) {
                              ArtSweetAlert.show(
                                context: context,
                                artDialogArgs: ArtDialogArgs(
                                  type: ArtSweetAlertType.success,
                                  title: "Add Pembelian Successful",
                                  confirmButtonColor: Color.fromARGB(255, 3, 192, 0),
                                  onConfirm: () {
                                    Get.back();
                                    Get.back();
